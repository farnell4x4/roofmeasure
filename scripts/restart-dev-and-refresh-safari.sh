...
# Stop anything currently listening on the local dev port.
...
# Clean only Next.js local build/cache output.
echo "Cleaning local Next.js build..."
rm -rf .next

# Start the normal local development server.
echo "Starting Roof Tape Measure local dev server..."
...
# Open or refresh Roof Tape Measure in Safari.
osascript <<EOF
...
echo "Roof Tape Measure local dev server restarted and Safari refreshed at $APP_URL"
