...
describe("project measurement geometry", () => {
  ...
  it("uses the latest orphan point when older stray points exist", () => {
    ...
    const geometry = toProjectMeasurementData(
      [{ id: "segment-a", start: segmentStart, end: segmentEnd }],
      latestOrphan,
    )
  ...
  it("prefers canonical saved measurement geometry when present", () => {
    ...
    project.measurementGeometry = {
      segments: [
        { id: "segment-canonical", start: canonicalStart, end: canonicalEnd },
      ],
  ...
  it("keeps the canonical geometry signature unchanged when only the visual zoom changes", () => {
    const segments = [
      {
        id: "segment-a",
        start: { latitude: 39.1000001, longitude: -105.2000001 },
        end: { latitude: 39.1005001, longitude: -105.2005001 },
      },
    ]
    const pending = { latitude: 39.1008001, longitude: -105.2008001 }

    const beforeVisualZoom = measurementGeometrySignature(segments, pending)
    const afterVisualZoom = measurementGeometrySignature(segments, pending)

    expect(afterVisualZoom).toBe(beforeVisualZoom)
  })
})
