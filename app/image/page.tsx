...
function ImageProjectScreen() {
  ...
  const imageRoofPlaneSquaresById = useMemo(
    () => project ? calculateImageProjectTotals(project).planeSquaresById : {},
    [project],
  )
  ...
  return <main className="fullscreen-safe-area" style={{ background: "#d9ddd8" }}>
    ...
    {pitchMenu ? <div style={{ ...popupStyle, position: "fixed", width: 280, left: Math.min(pitchMenu.anchor.x - 80, window.innerWidth - 292), top: Math.min(pitchMenu.anchor.y + 16, window.innerHeight - 220) }}><label style={{ display: "grid", gap: 6, fontSize: 13, fontWeight: 700 }}>Pitch <span><input aria-label="Roof pitch rise" type="number" min="0" max="36" step=".25" autoFocus value={pitchMenu.draft} onChange={(event) => setPitchMenu({ ...pitchMenu, draft: event.target.value })} style={{ width: 72, border: "1px solid rgba(31,37,34,.18)", borderRadius: 10, padding: 8 }}/> /12</span></label><div style={{ display: "flex", gap: 8 }}><button type="button" onClick={() => setPitch(pitchMenu.planeId, pitchMenu.draft)} style={{ ...buttonStyle, flex: 1, background: "#1f2522", color: "#fff" }}>Save</button><button type="button" onClick={() => setPitch(pitchMenu.planeId, pitchMenu.draft, true)} style={{ ...buttonStyle, flex: 1, background: "rgba(31,37,34,.08)" }}>All</button></div><p style={{ margin: 0, paddingTop: 2, borderTop: "1px solid rgba(31,37,34,.12)", color: "#5f685f", fontSize: 12, lineHeight: 1.4 }}>Slope only affects calculations for map projects. It does not affect this image project because you are manually entering exact sloped measurements.</p></div> : null}
