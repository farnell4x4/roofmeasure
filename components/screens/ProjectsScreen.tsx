  ...
  const filtered = useMemo(() => {
    ...
    if (sort === "address") {
      return items.sort((left, right) =>
        (left.location?.formattedAddress ?? "").localeCompare(right.location?.formattedAddress ?? "")
      );
    }
    return items;
  }, [projects, query, sort]);
    ...
    <main className="app-shell page-grid">
      <section style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
        <div>
          ...
          <p style={{ color: "var(--muted)", margin: 0 }}>Search, sort, duplicate, export, or reopen any locally stored roof measurement project.</p>
        </div>
      ...
      <Card style={{ display: "grid", gap: 12 }}>
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
          <label>
            <input className="sr-only" type="file" accept="application/json" onChange={handleImportProject} />
          ...
          return (
            <Card key={project.id} style={{ display: "grid", gap: 14 }}>
              ...
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap", color: "var(--muted)", fontSize: 14 }}>
                <span>Modified {new Date(project.updatedAt).toLocaleString()}</span>
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                ...
                <Button variant="secondary" onClick={() => handleExportProject(project.id)}>
