...
export function MapViewport({
      ...
      try {
        ...
        const handleRegionChange = () => {
          ...
          onCameraChangeRef.current({
            ...
            centerLng:
              currentRegion.center.longitude ?? initialCamera.centerLng,
        ...
        if (cancelled) return
        setMapError(
          getMapKitRuntimeErrorMessage(error) ||
            (await getMapKitConfigurationErrorMessage()),
        )
      }
    }
  ...
  useEffect(() => {
    ...
    const changed = nextValues.some(
      (value, index) => Math.abs(value - currentValues[index]) > 0.000001,
    )
    if (!changed) return

    const nextRegion = createMapKitRegion(
      camera.centerLat,
      camera.centerLng,
      camera.latSpan,
      camera.lngSpan,
  ...
  const projectedPoints = useMemo(() => {
    const map = mapInstanceRef.current
    const mapkit = typeof window === "undefined" ? undefined : window.mapkit
    if (!map || !mapkit || pageRect.width === 0 || pageRect.height === 0)
      return null
...
function SegmentLine({
  ...
  const color =
    MEASUREMENT_TYPES.find((item) => item.type === segment.type)?.color ??
    "#fff"
