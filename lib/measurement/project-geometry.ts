...
import { EditableMeasurementPoint as MeasurementPoint, EditableMeasurementSegment, Project } from "@/types/models";
...
export function fromProjectMeasurementData(project: Project) {
  ...
  const segments = project.segments
    ...
    .filter((segment): segment is EditableMeasurementSegment => Boolean(segment));

  const orphanPoint = [...project.points].reverse().find((point) => !usedPointIds.has(point.id));
