...
export const db = {
  ...
  async saveProject(project: Project) {
    ...
    const transaction = database.transaction([PROJECTS_STORE, RECOVERY_STORE], "readwrite");
  ...
  async getPreferences() {
    const database = await getDatabase();
    return (await database.get(PREFERENCES_STORE, "app")) as AppPreferences | undefined;
  },
  ...
  async getRecovery(projectId: string) {
    const database = await getDatabase();
    return (await database.get(RECOVERY_STORE, projectId)) as RecoveryEntry | undefined;
  }
};
